	...
	var Core = {
		...
		**/
		setupSliders: function() {
			...
			if( $("#portfolio-pager").length ) {
				$("#portfolio-pager").thumbnailScroller({ 
		...
		**/
		setupCarousels: function() {
			...
			$('.template-home-parallax .portfolio .items').each( function() {
		...
		setupHomepages: function() {
			...
			**/
			if( $('#blog-posts-home').length ) {
				$('#blog-posts-home').thumbnailScroller({
					scrollerType:"clickButtons", 
					scrollerOrientation:"horizontal", 
					scrollEasing:"easeOutCirc", 
					scrollEasingAmount:800, 
		...
		**/
		navMenu: function() {
			...
			if( $('.big-header-wrapper').length && $('html').hasClass('mobile') == false ) {
